					...
					while (sdrCongRef.Read())
					{
						...
						*/
						...
						if ((dicConfRefRow["ActualField"] == "1") || (dicConfRefRow["ActualField"] == "2"))
						{
							...
							{
								strSubQueryJoinField = string.Format("{0}.[{1}]=ActualNK.[{1}]", dicConfRefRow["SrcTable"], dicConfRefRow["SrcField"]);
							}
							else
							{
								strSubQueryJoinField += string.Format(" AND {0}.[{1}]=ActualNK.[{1}]", dicConfRefRow["SrcTable"], dicConfRefRow["SrcField"]);
							}
						}
						strInnerJoin = string.Format("INNER JOIN (SELECT {0}, {2} FROM {1} GROUP BY {2}) AS ActualNK ON {3}", strSubQueryGroupByField, dicConfRefRow["SrcTable"], strSubQuerySelectField, strSubQueryJoinField);
						///
